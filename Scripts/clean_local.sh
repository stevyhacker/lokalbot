...
# starts from a clean slate. Scoped to BotinaV2/LokalBot artifacts only — it
# never touches installed apps, other projects' DerivedData, or TCC grants.
# Usage: bash Scripts/clean_local.sh
set -euo pipefail
cd "$(dirname "$0")/.."

echo "=== BotinaV2 local build cleanup ==="
...
# Eject mounted BotinaV2 volumes first so nothing holds a file open while we
...
remove "/tmp/BotinaV2-dmg-venv" # throwaway dmgbuild venv from build_test_dmg.sh

# DMG images at the repo root and the test-DMG temp location.
shopt -s nullglob
for dmg in ./*.dmg /tmp/BotinaV2*.dmg; do
  remove "$dmg"
done
