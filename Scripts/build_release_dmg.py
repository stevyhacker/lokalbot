#!/usr/bin/env python3
"""Build a styled release DMG for LokalBotV3 with dmgbuild.
...
Flow:
1. Validate the built `LokalBotV3.app` (auto-discovered when --app is omitted).
...
# Product constants. LokalBotV3 ships as a single window; keep the geometry
# compact so the mounted volume opens without scrollbars on a laptop display.
VOLUME_NAME = "LokalBotV3"
APP_BUNDLE_NAME = "LokalBotV3.app"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Build a styled LokalBotV3 release DMG with dmgbuild."
    )
    parser.add_argument(
        "--app",
        default=None,
        help=(
            "Path to the built LokalBotV3.app. When omitted, the newest bundle "
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Path for the final DMG. Defaults to build/LokalBotV3.dmg.",
    )
    parser.add_argument(
        "--background",
        default=None,
...
def main() -> int:
    ...
    output_path = (
        ...
        if args.output
        else repo_root() / "build" / "LokalBotV3.dmg"
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="LokalBotV3-dmgbuild-") as temporary_root:
