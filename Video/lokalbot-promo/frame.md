...
description: >
  ...
  SF Pro Display in lowercase weight 900 treated as graphic primitive, SF Mono chrome (uppercase,
...
typography:
  ...
  label:   { fontFamily: "SF Mono", cqw: 0.72, weight: 500, tracking: "0.14em", upper: true }
...
- **Fonts** — already set to **SF Pro Display** (display) / **SF Mono** (body); ignore any preset font name lingering in prose.
...
and size, not face contrast. **SF Mono** is chrome only (numbers, kickers, tags, axis labels,
...
- **SF Mono chrome** — uppercase, 0.14em; the `/` bullet marker; mono catalogue numbers.
...
- Keep chrome in **SF Mono uppercase, 0.14em**; use the `/` mono bullet marker.
...
- **SF Pro Display + SF Mono ship as local files in `assets/fonts/`** (SF-Pro-Display-{Regular,Medium,Semibold,Bold,Heavy,Black}.otf, SF-Mono-{Regular,Medium,Bold}.otf) — declare `@font-face` against those exact paths; never a network `@import` or a Google Fonts URL. All visible copy is Latin; no CJK fallback is needed.
