...
/// always written. The legacy `lokalbotv2Log(_:)` shim is rerouted to ``line(_:)``
/// so existing call sites keep working unchanged.
///
/// Deliberately NOT main-actor isolated: `lokalbotv2Log` is a free function
...
enum AppLog {
    ...
    /// entry point the rerouted `lokalbotv2Log(_:)` calls, preserving the old
    ...
    /// `<App Support>/com.dotenv.LokalBotV2/debug.log` — the same file the app has
