...
struct LokalBotCLIInstaller {
    var home: URL
    /// The embedded CLI at `LokalBot.app/Contents/Helpers/lokalbot-cli`, nil
    /// when the bundle hasn't been built with CLI embedding enabled.
    var bundledBinary: URL?
    /// The embedded skill folder at `LokalBot.app/Contents/Resources/lokalbot-cli/`,
    ...
    /// stale `LokalBot.app` copy) reads as "not installed" so the UI re-offers
    ...
    var isBundleLocationStable: Bool {
        ...
        let bundle = binary
            ...
            .deletingLastPathComponent()  // .../LokalBot.app
    ...
    func uninstall() throws {
        for link in [binLink, skillLink] {
            ...
            // Only remove links that resolve into a LokalBot app bundle - never
            // foreign symlinks at our canonical paths. Keep legacy cleanup for
            // installs created before the visible app rename.
