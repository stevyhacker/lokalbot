...
struct LokalBotCLIInstaller {
    var home: URL
    /// The embedded CLI at `BotinaV2.app/Contents/Helpers/lokalbot-cli`, nil
    /// when the bundle hasn't been built with CLI embedding enabled.
    var bundledBinary: URL?
    /// The embedded skill folder at `BotinaV2.app/Contents/Resources/lokalbot-cli/`,
    ...
    /// stale `BotinaV2.app` copy) reads as "not installed" so the UI re-offers
    ...
    var isBundleLocationStable: Bool {
        ...
        let bundle = binary
            ...
            .deletingLastPathComponent()  // …/BotinaV2.app
    ...
    enum InstallError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .bundleNotStable:
                return "The app is running from a translocated or read-only location. Move BotinaV2.app to /Applications and relaunch."
    ...
    func uninstall() throws {
        for link in [binLink, skillLink] {
            ...
            // Only remove links that resolve into *some* BotinaV2.app — never
            // foreign symlinks at our canonical paths.
            if destPath.contains("BotinaV2.app/") {
    ...
    func addLocalBinToPath() throws {
        ...
        updated += "\n# Added by BotinaV2 / lokalbot-cli\n\(Self.pathExportLine)\n"
        do {
