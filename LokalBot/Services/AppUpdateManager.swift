...
final class AppUpdateManager: ObservableObject {
    ...
    private static let debugCheckOnLaunchArgument = "-botinav2-check-for-updates-on-launch"
    ...
    func start() {
        ...
        guard Self.isUpdaterEnabledForThisBuild else {
            botinav2Log("Sparkle disabled for dev build."); return
        }
        guard hasUsableConfiguration else {
            botinav2Log("Sparkle not started: updater config incomplete (see RELEASING.md).")
            return
        }
    ...
    func checkForUpdates() {
        guard isStarted else {
            botinav2Log("Ignoring manual update check; updater not started.")
            return
        }
        updaterController.checkForUpdates(nil)
    }

    /// Compiled to `false` in the dev configuration (BOTINA_DEV), which ships a
    ...
    private var hasUsableConfiguration: Bool {
        guard let feed = configuredString("SUFeedURL"),
            ...
            botinav2Log("Sparkle: missing/placeholder SUFeedURL.")
            return false
        }
        guard let key = configuredString("SUPublicEDKey"), key != Self.publicKeyPlaceholder else {
            botinav2Log("Sparkle: missing/placeholder SUPublicEDKey.")
            return false
        }
        return true
    }
