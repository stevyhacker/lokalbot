...
**Open-source AI meeting notes, dictation, and inline autocomplete for your Mac — on-device by default.**

Records both sides of the call — no bot joining — and writes the recap with local models. No account, no telemetry, no LokalBot cloud. Don't take that on faith: with the built-in backend, required models already downloaded, and automatic update checks off, [point Little Snitch at it](#privacy--verify-it) through a full record → transcribe → summarize cycle and watch the processing path stay silent.
...
<sub>Meeting recap → speaker-labeled transcript → search → day timeline → inline autocomplete — running locally.</sub>
...
LokalBot records both sides of your calls — your mic is **Me**, a Core Audio tap on the meeting app is **Them** — so speaker labels come free and no bot ever joins. Transcription, summaries, and search run on your Mac by default.

Around that core: **Cotyping** (inline autocomplete in any app), **Dictation** (hold **⌥ Space**, talk), a private visual day rewind, screenshot-backed answers, system-wide **Quick Recall**, chat over your local memory, and an embedded coding agent that can run against your local Main LLM.
...
- **Recall with evidence.** Search OCR by meaning, app, or date; open the exact encrypted frame behind an answer; save important moments; and scrub or play the day as a visual rewind.
- **Private by construction.** Optional screenshots are AES-GCM encrypted and auto-delete after 14 days unless you explicitly save a moment; excluded apps are never captured. Quick Recall is opt-in, and external screen-memory tools have their own permission separate from meeting access.
...
Embeddings live in SQLite with brute-force cosine similarity — at personal scale, brute force is instant.

## Privacy — verify it

Privacy is the architecture, not a slogan. Audio, transcripts, summaries, embeddings, screenshots, and activity live in local files and SQLite under your account. There is no account or telemetry, and audio is never sent to a LokalBot service.
...
Apple Silicon (M1 and later) on macOS 15.0+, by design: LokalBot is welded to the Neural Engine, MLX, and Core Audio process taps. There is no Intel, Windows, or Linux build planned.
</details>
...
Only if you turn on screenshots — they're off by default and opt-in during onboarding. When on, they're AES-GCM encrypted on disk and deleted after 14 days by default; a moment you explicitly save is retained until you unsave or delete it. Excluded apps are never captured. Capture is display-level, so LokalBot cannot identify or redact an individual password field inside an otherwise allowed app; exclude any app whose pixels should never be stored. Cotyping separately refuses password and secure fields.
</details>
...
If LokalBot is useful to you, a star genuinely helps other people find it.
