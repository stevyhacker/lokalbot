# LokalBot — M1 recorder · M2 transcribe & summarize · M3 search · M4 day tracking
...
- **Headless mode:** `LokalBot --process <meeting-folder> [--no-summary]` runs the pipeline without the UI (used for end-to-end testing).

**M3 — search & player**
- **Index:** SQLite + FTS5 (`lokalbot.sqlite` in the storage root, system SQLite — no dependency) over titles, transcript segments, and summaries. Segment-level rows carry their audio timestamp. Incremental re-index by file mtime, triggered on launch and after each pipeline run.
...
- **Headless:** `LokalBot --search "<query>"` prints index hits (test hook).
