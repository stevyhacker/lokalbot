...
Around that core: **Cotyping** (inline autocomplete in any app), **Dictation** (hold **⌥ Space**, talk), a private day timeline, chat over your meeting library, and an embedded coding agent that can run against your local Main LLM.
...
- **Private by construction.** Optional screenshots are AES-GCM encrypted and auto-delete after 14 days; excluded apps are never captured. Cotyping separately refuses password and secure fields.
...
`lokalbot-cli` (embedded in the app bundle) gives coding agents **read-only** access to your meeting library: `list` / `get` / `search` / `path`, JSON by default. The same binary is a stdio **MCP server** — `list_meetings`, `get_meeting`, `search_meetings`, and `ask_library`; the last answers through the app's local llama-server before returning the result to the requesting client. LokalBot itself does not upload library content, but an MCP client such as Claude Desktop or Cursor may transmit tool inputs and results under its own privacy terms, so connect only clients you trust. The whole surface is **off by default** behind the Privacy-pane toggle, and `Scripts/build-mcpb.sh` wraps it into a one-click `LokalBot.mcpb` for GUI MCP clients.
