...
- **UI** (`LokalBotUITests`, XCUITest): `Scripts/ui-tests.sh`. Drives the dedicated `LokalBotV3 UI Test Host.app` against a synthetic library planted under a tmp `LOKALBOTV3_STORAGE_ROOT`; `LOKALBOTV3_UI_TEST=1` makes the host skip every side-effectful subsystem (Core Audio polling, the accessibility-trusted detector, Sparkle, screenshots), so no app TCC grants are needed and the suite never touches the installed production app. Eighteen tests cover meeting-list grouping (+ Record button), sidebar navigation, Models, Settings + permission repair, calendar gating, the Chat section and persisted history, detail tabs (Summary/Transcript), FTS5 search → deep-link, timeline populated/empty states, cotyping settings gating, multi-select state, and both branches of the delete dialog (cancel keeps files; confirm removes rows **and** on-disk folders). The first run needs the controlling terminal/IDE to hold **Automation → Xcode** and **Accessibility** grants, or XCUITest fails with "Timed out while enabling automation mode."
