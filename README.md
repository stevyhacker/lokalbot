# LokalBotV1 — M1 recorder · M2 transcribe & summarize · M3 search · M4 day tracking
...
- **Storage:** `~/Library/Application Support/com.dotenv.LokalBotV1/meetings/YYYY/MM/dd-slug/` with `meta.json` per meeting. (Bundle-id folder, not "LokalBotV1" — avoids colliding with any other app's `Application Support/LokalBotV1`.)
...
- **Headless mode:** `LokalBotV1 --process <meeting-folder> [--no-summary]` runs the pipeline without the UI (used for end-to-end testing).

**M3 — search & player**
- **Index:** SQLite + FTS5 (`lokalbotv1.sqlite` in the storage root, system SQLite — no dependency) over titles, transcript segments, and summaries. Segment-level rows carry their audio timestamp. Incremental re-index by file mtime, triggered on launch and after each pipeline run.
...
- **Headless:** `LokalBotV1 --search "<query>"` prints index hits (test hook).
...
- **Dev/prod split:** `project.yml` builds `LokalBot` (prod, `com.dotenv.LokalBotV1`) and `LokalBot Dev` (`com.dotenv.LokalBotV1.dev`, `LOKALBOTV1_DEV`) from one target template, so running from Xcode holds its own Mic/Screen/Accessibility TCC grant and never disturbs the released app — and Sparkle is compiled out of dev.
...
- **Logging:** `lokalbotv1Log` now routes through swift-log (`AppLog`) → stdout + a rotating `debug.log` (`FileLogHandler`).
